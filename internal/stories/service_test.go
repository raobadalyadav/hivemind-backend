...
func TestStories_AudienceAndExpiry(t *testing.T) {
	...
	svc := NewService(NewRepository(pool), screener{}).WithMedia(fm)
	...
	newMedia := func(uid string) string { return fm.Add(uid, "image").ID }
	authorMedia := newMedia(author)
	for name, bad := range map[string]*Story{
		"no media":             {AuthorID: author, Audience: "connections"},
		"a link is not media":  {AuthorID: author, MediaID: "https://x/s.jpg", Audience: "connections"},
		"someone else's media": {AuthorID: stranger, MediaID: authorMedia, Audience: "connections"},
		"huge edits":           {AuthorID: author, MediaID: newMedia(author), Audience: "connections", Edits: `{"x":"` + strings.Repeat("a", MaxEditsBytes) + `"}`},
		"edits not an object":  {AuthorID: author, MediaID: newMedia(author), Audience: "connections", Edits: `[1,2]`},
		"unknown audience":     {AuthorID: author, MediaID: newMedia(author), Audience: "everyone"},
		"community w/o id":     {AuthorID: author, MediaID: newMedia(author), Audience: "community"},
		"id on connections":    {AuthorID: author, MediaID: newMedia(author), Audience: "connections", CommunityID: comm},
		"non-member community": {AuthorID: stranger, MediaID: newMedia(stranger), Audience: "community", CommunityID: comm},
		"screened caption":     {AuthorID: author, MediaID: newMedia(author), Audience: "connections", Caption: "BADWORD"},
	} {
	...
	conn, err := svc.CreateStory(ctx, &Story{AuthorID: author, MediaID: newMedia(author), Audience: "connections", Caption: "hi"})
	if err != nil {
		t.Fatalf("connections story: %v", err)
	}
	if conn.MediaType != "image" || conn.Width != 1080 || conn.ThumbURL == "" || conn.Edits != "{}" {
		t.Fatalf("story must carry the upload's kind/size/thumbnail: %+v", conn)
	}
	withEdits, err := svc.CreateStory(ctx, &Story{AuthorID: author, MediaID: newMedia(author), Audience: "connections", Edits: `{"filter":"warm","overlays":[{"t":"hi","x":0.5,"y":0.5}]}`})
	if err != nil || !strings.Contains(withEdits.Edits, "warm") {
		t.Fatalf("edits JSON is stored: %+v err=%v", withEdits, err)
	}
	if got := conn.ExpiresAt.Sub(conn.CreatedAt); got != Lifetime {
		t.Fatalf("expiry is fixed at 24h server-side, got %v", got)
	}
	comStory, err := svc.CreateStory(ctx, &Story{AuthorID: author, MediaID: newMedia(author), Audience: "community", CommunityID: comm})
	if err != nil {
		t.Fatalf("community story: %v", err)
	}
	archived, _ := svc.CreateStory(ctx, &Story{AuthorID: author, MediaID: newMedia(author), Audience: "connections", KeepArchive: true})
	...
	if live, _ := svc.ListMyStories(ctx, author, false); len(live) != 0 {
		t.Errorf("live list should be empty: %d", len(live))
	}
	arch, _ := svc.ListMyStories(ctx, author, true)
	if len(arch) != 1 || arch[0].ID != archived.ID {
		t.Errorf("archive should hold only the keep_archive story: %+v", arch)
	}
